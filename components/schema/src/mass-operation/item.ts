import { z } from 'zod';
import { ComponentInputSchema } from '../item/components';
...
export const UpdateItemOperationSchema = CreateItemOperationSchema.omit(
    { action: true, type: true, shape: true, }).merge(z.object({
        action: z.literal('update'),
    }))

export const UpsertItemOperationSchema = UpdateItemOperationSchema.omit(
    { action: true }).merge(z.object({
        action: z.literal('upsert'),
    }))

export const UpdateCompomentOperationSchema = UpdateItemOperationSchema.omit(
    { action: true, components: true }).merge(z.object({
